...
export default {
    ...
    async execute(
        ...
        const inputFields = input.split(' ');
        const command = inputFields[0].toLowerCase();
        if (
            ['sessions', 'whitelist'].includes(command) &&
            !_discordClient.isReady()
        ) {
        ...
        switch (command) {
            ...
            case 'whitelist': {
                const guildId = inputFields[1];
                const feature = inputFields[2];
                const duration = inputFields[3];
                let durationMs = -1;
                if (!guildId || !feature) {
                    console.log(
