    ...
    async execute(
        ...
        if (
            ...
            (
                ...
                discordClient.guilds.cache.get(guildId).members.me.voice
                    .channelId
        ) {
        ...
        switch (item.type) {
            case UpdateItemType.Add: {
                ...
                let me = await discordClient.guilds.cache
                    .get(guildId)
                    .members.fetchMe();
                ...
                if (
                    discordClient.music.ws.state !== LavalinkWSClientState.Ready
                ) {
                ...
                const result =
                    await discordClient.music.api.loadTracks(searchQuery);
                ...
                let player = (await discordClient.music.players.fetch(
                    guildId,
                )) as QuaverPlayer;
                if (!player?.voice.connected) {
                    player = discordClient.music.players.create(
                        guildId,
                    ) as QuaverPlayer;
                    player.handler = new PlayerHandler(discordClient, player);
                    ...
                    me = await discordClient.guilds.cache
                        .get(guildId)
                        .members.fetchMe();
                    ...
                    io.to(`guild:${guildId}`).emit(
                        ...
                        player.queue.tracks.map(
                            (track: QuaverSong): QuaverSong => {
                                const user = discordClient.users.cache.get(
                                    track.requesterId,
                                );
            ...
            case UpdateItemType.Loop: {
                const player = (await discordClient.music.players.fetch(
                    guildId,
                )) as QuaverPlayer;
                if (!player) {
